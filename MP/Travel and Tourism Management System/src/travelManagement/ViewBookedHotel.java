...
public class ViewBookedHotel extends JFrame implements ActionListener{
	JLabel cusername,cuname,id,number,name,gender,country,address,phone,email,txt,days,ndays,acnac,food,cfood,cacnac;
	JButton back;
	public ViewBookedHotel(String username) {
		...
		txt=new JLabel("VIEW BOOKED HOTEL DETAILS");
		txt.setFont(new Font("Forte",Font.BOLD,30));
		txt.setBounds(60,0,550,30);
		add(txt);
		...
		days.setFont(new Font("Matura MT Script Capitals",Font.BOLD,20));
		days.setBounds(30,170,150,25);
		add(days);
		
		JLabel ndays=new JLabel();
		ndays.setBounds(220,170,150,25);
		add(ndays);
		
		acnac=new JLabel("AC/Non-AC");
		acnac.setFont(new Font("Matura MT Script Capitals",Font.BOLD,20));
		acnac.setBounds(30,210,200,25);
		add(acnac);
		
		JLabel cacnac=new JLabel();
		cacnac.setBounds(220,210,200,25);
		add(cacnac);
		
		food=new JLabel("Food Included?");
		food.setFont(new Font("Matura MT Script Capitals",Font.BOLD,20));
		food.setBounds(30,250,200,25);
		add(food);
		
		JLabel cfood=new JLabel();
		cfood.setBounds(220,250,200,25);
		add(cfood);
		
		name=new JLabel("Id");
		name.setFont(new Font("Matura MT Script Capitals",Font.BOLD,20));
		name.setBounds(30,290,150,25);
		add(name);
		
		JLabel nm=new JLabel();
		nm.setBounds(220,290,150,25);
		add(nm);
		
		gender=new JLabel("Number");
		gender.setFont(new Font("Matura MT Script Capitals",Font.BOLD,20));
		gender.setBounds(30,330,150,25);
		add(gender);
		
		JLabel gdr=new JLabel();
		gdr.setBounds(220,330,150,25);
		add(gdr);
		
		country=new JLabel("Phone");
		country.setFont(new Font("Matura MT Script Capitals",Font.BOLD,20));
		country.setBounds(30,370,150,25);
		add(country);
		
		JLabel cty=new JLabel();
		cty.setBounds(220,370,150,25);
		add(cty);
		
		address=new JLabel("Total Cost");
		address.setFont(new Font("Matura MT Script Capitals",Font.BOLD,20));
		address.setBounds(30,410,150,25);
		add(address);
		
		JLabel adde=new JLabel();
		adde.setBounds(220,410,150,25);
		add(adde);
        ...
        try {
            ...
            PreparedStatement ps1 = con.prepareStatement("select * from bookhotel where username = ?");
            ps1.setString(1, username); // Set the username parameter
            ResultSet rs = ps1.executeQuery();
            while (rs.next()) {
                cuname.setText(rs.getString("username"));
                uid.setText(rs.getString("name"));
                ...
                cfood.setText(rs.getString("food"));
                cacnac.setText(rs.getString("ac"));
                ndays.setText(rs.getString("days"));
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
	...
	public static void main(String[] args) {
		new ViewBookedHotel("The Unique One !");
	}
